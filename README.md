...
## 2. 最新ワークフロー：統合知性による意味照合

第十三次実験を経て、SigmaSenseの思考プロセスは、複数の論理エンジンを統合した、より洗練されたワークフローへと進化した。現在の主要な処理 (`sigma_sense.py` の `match` メソッド) は、以下のステップで実行される。

1.  **特徴の観測 (`DimensionGenerator`)**: 複数の画像分析エンジン（OpenCV, EfficientNet, MobileViT等）を並列で実行し、画像から「主要色の彩度」「輪郭の揺らぎ」といった基本的な特徴量を網羅的に抽出する。

2.  **パターンの示唆 (`LogicalPatternSuggester`)**: 観測された特徴の組み合わせを分析し、「もし毛皮があり、四本足なら、犬科の動物かもしれない」といった、ルールブックにない新たな仮説（追加の事実）をゼロショットで提案する。

3.  **常識による推論 (`SymbolicReasoner`)**: 観測された事実と提案された仮説を元に、`common_sense_rulebase.json` に定義された常識ルール（例：「もし犬なら、動物である」）を適用し、さらなる事実を演繹的に導き出す。

4.  **論理式の評価 (`LogicalExpressionEngine`)**: 全ての事実が出揃った段階で、各意味次元に定義された論理式（例：`is_canine = (is_dog OR is_wolf)`）を評価し、最終的な論理的状態を確定させる。

5.  **例外の適用 (`ContextualOverrideEngine`)**: 最終確定の直前に、「ペンギンは鳥だが、飛べない」のような例外ルールを適用し、推論の矛盾を解消する。

6.  **意味ベクトルの構築**: 確定した論理的状態に基づき、最終的な37次元の意味ベクトルを構築する。

7.  **データベース照合 (`FastMatchPredictor`)**: 構築されたベクトルをデータベース内の全ベクトルと照合する。この際、`FastMatchPredictor` が事前に候補を絞り込むことで、計算効率を向上させている。

8.  **語りの生成 (`NarrativeJustifier`)**: 最終的な照合結果と、そこに至るまでの思考プロセス（観測→示唆→推論→例外適用）全体を、人間が理解できる自然言語の「語り」として生成する。

この一連のプロセスにより、SigmaSenseは単なるパターンマッチングではなく、複数の思考エンジンを協調させた、多角的でロバストな意味照合を実現している。
...
1.  **診断医 (`tools/functor_consistency_checker.py`)**:
    テスト画像に様々な変換（回転、色調変更など）を加え、その前後で意味ベクトルがどのように変化したかを精密に検査します。もし、ある変換が意図しない次元にまで影響を与えていた場合、それを「一貫性の破綻」として検出します。
...
- `run_sigma.py`: 基本的な意味照合タスクを実行します。
- `run_learning_objective.py`: 外部から与えられた学習目標を処理するワークフローを開始します。

### B. コアエンジン
SigmaSenseの思考と判断の中核を担うモジュール群です。

- `sigma_sense.py`: **統合思考オーケストレータ**。特徴生成から、提案、推論、例外処理、ベクトル化、照合、語り生成まで、一連の思考プロセス全体を制御する。
...
### F. 高度機能
システムの基本機能に加えられた、専門的な能力です。

- **F1. 倫理フィルター (イージス)**
  - `aegis_ethics_filter.py`: 生成された情報が倫理規定に抵触しないか検査し、公開を制限します。
- **F2. メタ認知 (豊川モデル)**
  - `toyokawa_model.py`: システム全体の心理状態 `C(t)` を計算する数式モデル。
...
- `common_sense_rulebase.json`: `SymbolicReasoner` が使用する常識ルールブック。

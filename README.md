...
#### 演繹エンジンの理論的基盤
- **構造幾何学 (Structural Geometry)**: OpenCVライブラリを駆使し、**フーリエ記述子**や**Huモーメント不変量**といった手法で、画像の形状、輪郭、空間配置といった回転・スケール・位置・**射影変換**に不変な本質的特徴量を抽出します。
- **線形代数 (Linear Algebra)**: 抽出された特徴は、高次元ベクトル空間の「意味ベクトル」として表現されます。類似度の計算には、**コサイン類似度**に加え、**KLダイバージェンス**や**ワッサースタイン距離**といったより厳密な情報幾何学・最適輸送理論に基づく手法も利用可能になりました。すべての意味的な距離や関係性は、このベクトル空間上の幾何学として厳密に定義されます。
- **情報理論 (Information Theory)**: 各ベクトルが持つ情報量を**エントロピー**や**スパース度**として定量化します。さらに、異なる意味ベクトル間の**相互情報量**を計算することで、それらの間の意味的な関連性をより深く評価できるようになりました。これにより、システムは自身の「理解度」を自己評価し、情報が不十分な場合は**再構成理論**に基づき、ベクトルを補完するプロセスを自らトリガーします。
...
- **構造幾何学 (Structural Geometry)**:
  - **内容**: `cv2.moments`や`cv2.HuMoments`を用いて、画像の形状を回転・スケール・**射影変換**に不変な特徴量として抽出しています。
  - **該当ファイル**: `sigma_image_engines/engine_opencv.py`, `shape_descriptor_extractor.py`, `tests/test_group_theory.py`

- **線形代数 (Linear Algebra)**:
  - **内容**: `weighted_cosine_similarity`（重み付きコサイン類似度）に加え、KLダイバージェンスやワッサースタイン距離といった情報幾何学・最適輸送理論に基づく類似度計算も可能になりました。
  - **該当ファイル**: `sigma_sense.py`, `sigma_local_core.py`, `information_metrics.py`, `tests/test_sigma_sense_logic.py`

- **情報理論 (Information Theory)**:
  - **内容**: `compute_entropy`（エントロピー）、`compute_sparsity`（スパース度）に加え、異なる意味ベクトル間の**相互情報量**を計算し、その関連性を評価します。これらの結果に基づいて`should_trigger_reconstruction`が意味の再構成が必要かを判断しています。
  - **該当ファイル**: `information_metrics.py`, `reconstruction_trigger.py`, `tests/test_information_theory.py`
...
- **層理論 (Sheaf Theory)**:
  - **内容**: `SheafAnalyzer`が、`structure_detector`によって検出された画像の局所領域（開集合）が重なる部分で、それぞれの特徴ベクトルに矛盾がないか（貼り合わせ条件）を検証します。これにより、局所的な情報から矛盾のない大域的な特徴を「貼り合わせる」ことが可能になり、層理論の基本公理を実践しています。
  - **該当ファイル**: `src/sheaf_analyzer.py`, `src/structure_detector.py`

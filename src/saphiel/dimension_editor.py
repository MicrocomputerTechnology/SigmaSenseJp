...
if __name__ == '__main__':
    # Define project root and config directory for path construction
    import os
    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
    config_dir = os.path.join(project_root, 'config')
    
    input_file = os.path.join(config_dir, 'vector_dimensions_custom_ai.json')
    output_file = os.path.join(config_dir, 'vector_dimensions_edited.json')

    # Example usage:
    # This assumes 'vector_dimensions_custom_ai.json' exists and is readable.
    try:
        editor = DimensionEditor(input_file)
        ...
        print(f"\nSkipping example usage because {input_file} was not found.")

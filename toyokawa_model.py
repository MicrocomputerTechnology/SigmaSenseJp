...
class ToyokawaModel:
    ...
    def load_log(self, log_path="psyche_log.jsonl"):
        """Loads the psyche log file."""
        if not os.path.exists(log_path):
            raise FileNotFoundError(f"Psyche log file not found at '{log_path}'")
        log_data = []
    ...
    def run_analysis(self, log_path="psyche_log.jsonl"):
        """Runs the full analysis on the log file."""
        print_header(f"Running Toyokawa Model Analysis on '{log_path}'")
        try:
